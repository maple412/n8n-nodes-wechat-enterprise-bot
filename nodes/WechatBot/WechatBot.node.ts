...
export class WechatBot implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const mentionedAll = this.getNodeParameter('mentionedAll', i, false) as boolean;
				...
				if (!webhookKey) {
					throw new NodeOperationError(this.getNode(), '机器人Webhook Key不能为空');
				}

				if (!content) {
					throw new NodeOperationError(this.getNode(), '消息内容不能为空');
				}
				...
				if (response.data.errcode !== 0) {
					throw new NodeOperationError(
						this.getNode(),
						`企业微信API错误: ${response.data.errmsg} (错误码: ${response.data.errcode})`
					);
				}
